...
First, you need to download the `customers-tomcat-asis.war` file in the https://github.com/redhat-mw-demos/app-mod-projects/tree/main/packages-mta[packages-mta^] folder.
...
You will now be presented with options for packages. In order to accelerate the analysis, we will choose only the business packages related to our application. In the case of this application, these classes belong to the `io.konyeyor` package. You need to remove the `io` package from the *Included package* box by clicking on `<` button. Once you expand the `io` package in the *Packages* box, check only `io.konveyor` package to move to the *Included package* box by clicking on the `>` button.
...
Download the custom rule https://github.com/redhat-mw-demos/app-mod-projects/blob/main/customrules/corporate-framework-config.windup.xml[corporate-framework-config.windup.xml^]. Click on `Add Rule`.

* `Upload the provided rules file`  

image::../images/add-custom-rules.png[add-custom-rules]

Click on `Close`.

image::../images/complete-custom-rules.png[complete-custom-rules]

Click on `Next`. Then, you will now be presented with options for labels.
...
Once the report is finished, click on the link to access the report. Click on `customers-tomcat-asis.war` application.
