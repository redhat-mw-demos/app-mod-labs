= Enhance Applications with Managed Services - 30 minutes
...
===== [underline]#*Set Permissions to  "READ" messages from topics*#

To be able to view the messages from the broswer on console.redhat.com, you will need to allow Read Permissions for the Kafka topic.

. Click *Manage access* button in the same page,  choose `All Accounts` and click on *Next*
. Under the *Assign permissions* section click on the `Add permission` drop down, select the following options as show in the below screenshot +
+
image::../images/8-enhance-apps/all-allow-topic-read.png[]
