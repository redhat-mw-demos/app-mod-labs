...
- UI development teams and other API consumers start their development against the mocked api endpoints. 
...
*Manage and Secure the APIs* Use a n API management platform to expose the APIs in a secure and managed manner for access by the mobile app and other 3rd party applications.
...
[NOTE]
====
Here are instruction for how to access the Private Endpoint of the Product Catalog API.
